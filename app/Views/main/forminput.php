...
<div class="form-row">
    ...
    <div class="form-group col-md-6">
        ...
        <input type="date" class="form-control" name="tglpermintaan" id="tglpermintaan" value="<?= date('Y-m-d') ?>">
    </div>
</div>
...
<script type="text/javascript">
    ...
    function listdatabarangrestock() {
        var table = $('#barangrestock').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "/main/listdatabarangrestockFormInput",
                "type": "POST",
            },
            "columnDefs": [{
                "targets": [0, 7],
    ...
    function buatNoSuratJalan() {
        let tanggal = $('#tglpermintaan').val();
        $.ajax({
            type: "post",
            url: "/barangpermintaan/buatNoSuratJalan",
            data: {
                tanggal: tanggal
            },
            dataType: "json",
            success: function(response) {
                $('#nosuratjalan').val(response.nofaktur);
                dataTemp();
            },
            error: function(xhr, ajaxOptions, thrownError) {
                alert(xhr.status + "\n" + xhr.responseText + "\n" + thrownError);
    ...
    $(document).ready(function() {
        ...
        dataTemp();
        buatNoSuratJalan();
        $('#tglpermintaan').change(function(e) {
            buatNoSuratJalan();
        ...
        $('#tombolTambahItem').click(function(e) {
            ...
            if (nosuratjalan.length == 0) {
                ...
                $.ajax({
                    type: "post",
                    url: "/barangpermintaan/simpanTemp",
                    data: {
                        nosuratjalan: nosuratjalan,
                        kodebarang: kodebarang,
                        hargajual: hargajual,
            ...
            if (nosuratjalan.length == 0) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "post",
                            url: "/barangpermintaan/selesaiTransaksi",
                            data: {
                                nosuratjalan: nosuratjalan,
